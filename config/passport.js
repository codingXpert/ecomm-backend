...
passport.use(
  ...
  passport.serializeUser((user, done) => {   // used to save the specific data to req.session or req,body of the user
    done(null, user.id);
  });
  
  passport.deserializeUser((userId, done) => {   // takes the data that we serialize into the session and use it to retrieve data from db
